...
const createSquareIcon = (isSelected) =>
  L.divIcon({
    className: "relative",
    html: `
      <div class="relative flex flex-col items-center">
        <!-- Square Box -->
        <div class="w-10 h-10 bg-white border-4 ${
          isSelected ? "border-green-500" : "border-yellow-500"
        } flex items-center justify-center rounded-md shadow-lg">
          <img src="/evchargehub-logo.png" class="w-8 h-8" />
        </div>
        <!-- Triangle Pointer -->
        <div class="w-0 h-0 border-l-8 border-r-8 border-t-8 ${
          isSelected ? "border-t-green-500" : "border-t-yellow-500"
...
// Handles zooming into the selected station
    ...
    <MapContainer center={defaultPosition} zoom={13} className="h-full w-full rounded-lg">
      ...
      {stations.map((station) => (
        <Marker
          ...
          icon={createSquareIcon(selectedStation && selectedStation._id === station._id)}
          eventHandlers={{
